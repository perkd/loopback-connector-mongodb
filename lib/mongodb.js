...
MongoDB.prototype.connect = function(callback) {
  ...
  // CRITICAL FIX: Handle case where dataSource or settings is null after tenant cleanup
  if (!self.dataSource) {
    const error = new Error('DataSource is null - connection may have been cleaned up');
    if (callback) callback(error);
    return;
  }

  if (!self.settings) {
    const error = new Error('Settings is null - connector may have been cleaned up');
    if (callback) callback(error);
    return;
  }
  ...
  } else if (self.dataSource && self.dataSource.connecting) {
...
MongoDB.prototype.fromDatabase = function(modelName, data) {
  ...
  // CRITICAL FIX: Handle case where dataSource is null after tenant cleanup
  let modelInfo = this._models[modelName];
  if (!modelInfo && this.dataSource && this.dataSource.modelBuilder) {
    modelInfo = this.dataSource.modelBuilder.definitions[modelName];
  }

  // If we still don't have model info, return data as-is to prevent crashes
  if (!modelInfo) {
    console.log(`🔧 [MONGODB-FROMDATABASE-FIX] No model info found for ${modelName}, returning data as-is`);
    return data;
  }
...
MongoDB.prototype.toDatabase = function(modelName, data) {
  ...
  // CRITICAL FIX: Handle case where modelCtor is null after tenant cleanup
  if (!modelCtor) {
    console.log(`🔧 [MONGODB-TODATABASE-FIX] No model constructor found for ${modelName}, returning data as-is`);
    return data;
  }

  const props = modelCtor.properties;

  if (!this.settings || this.settings.enableGeoIndexing !== true) {
...
MongoDB.prototype.execute = function(modelName, command) {
  ...
  function doExecute() {
    ...
    try {
      // CRITICAL FIX: Get the correct database from current datasource context
      let effectiveDb = self.db;
      try {
        // Try to get the current datasource from the model
        const modelClass = self._models[modelName];
        if (modelClass && modelClass.model && typeof modelClass.model.getDataSource === 'function') {
          const currentDataSource = modelClass.model.getDataSource();
          if (currentDataSource && currentDataSource.connector && currentDataSource.connector.db) {
            effectiveDb = currentDataSource.connector.db;
            console.log(`🔧 [MONGODB-EXECUTE-FIX] Using database from current datasource: ${effectiveDb.databaseName}`);
          }
        }
      } catch (err) {
        console.log(`🔧 [MONGODB-EXECUTE-FIX] Could not get current datasource, using default: ${err.message}`);
      }

      // Use the effective database to get the collection
      if (!effectiveDb) {
        throw new Error(g.f('{{MongoDB}} connection is not established'));
      }
      const collectionName = self.collectionName(modelName);
      collection = effectiveDb.collection(collectionName);

      console.log(`🔧 [MONGODB-EXECUTE-FIX] Using collection from database: ${effectiveDb.databaseName}.${collectionName}`);
    } catch (err) {
      debug('Error: ', err);
      callback(err);
      return;
...
MongoDB.prototype.all = function all(modelName, filter, options, callback) {
  ...
  try {
    ...
    if (modelClass && modelClass.model && typeof modelClass.model.getDataSource === 'function') {
      ...
      if (currentDataSource && currentDataSource.connector && currentDataSource.connector.db) {
        effectiveDb = currentDataSource.connector.db;
        console.log(`🔧 [MONGODB-ALL-FIX] Using database from current datasource: ${effectiveDb.databaseName}`);
      }
    }
  } catch (err) {
    console.log(`🔧 [MONGODB-ALL-FIX] Could not get current datasource, using default: ${err.message}`);
  }

  // CRITICAL MONGODB TRACE: See what database is actually being queried
  console.log(`🔍 [MONGODB-TRACE] ===== MongoDB.all() called =====`);
  console.log(`🔍 [MONGODB-TRACE] Model: ${modelName}`);
  console.log(`🔍 [MONGODB-TRACE] Original database: ${self.db?.databaseName}`);
  console.log(`🔍 [MONGODB-TRACE] Effective database: ${effectiveDb?.databaseName}`);
  console.log(`🔍 [MONGODB-TRACE] Connection URL: ${self.settings?.url}`);
  console.log(`🔍 [MONGODB-TRACE] Collection: ${self.collectionName(modelName)}`);
...
function optimizedFindOrCreate(modelName, filter, data, options, callback) {
  ...
  // CRITICAL FIX: Get the correct database from current datasource context
  // This mirrors the fix in the execute() method for multitenant support
  let effectiveDb = self.db;
  let collection;
  ...
  try {
    ...
    if (modelClass && modelClass.model && typeof modelClass.model.getDataSource === 'function') {
      ...
      if (currentDataSource && currentDataSource.connector && currentDataSource.connector.db) {
        effectiveDb = currentDataSource.connector.db;
        if (self.debug) {
          debug('optimizedFindOrCreate: Using database from current datasource:', effectiveDb.databaseName);
        }
      }
    }
  } catch (err) {
    if (self.debug) {
      debug('optimizedFindOrCreate: Could not get current datasource, using default:', err.message);
    }
  }

  // Use the effective database to get the collection
  if (!effectiveDb) {
    return callback(new Error('MongoDB connection is not established'));
  }
  ...
  if (self.debug) {
    debug('optimizedFindOrCreate: Using collection from database:', effectiveDb.databaseName + '.' + collectionName);
  }
