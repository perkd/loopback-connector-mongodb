...
function MongoDB(settings, dataSource) {
  ...
  if (
    ...
    debug(
      'Optimized findOrCreate is enabled by default, and the ' +
      'enableOptimizedFindOrCreate setting is ignored since v7.0.0.',
    );
  }
...
MongoDB.prototype.connect = function(callback) {
  ...
  if (self.db) {
  ...
  } else if (dataSource && dataSource.connecting) {
    ...
    new mongodb.MongoClient(self.settings.url, validOptions).connect()
      .then(function(client) {
        if (self.debug) {
          debug('MongoDB connection is established: ' + self.settings.url);
        }
        self.client = client;
        const dbName = new URL(self.settings.url).pathname.replace(/^\//, '') ||
          self.settings.database;
        // client.db() inherits all MongoClient options (raw, readPreference,
        // readConcern, etc.) via Object.assign — no explicit dbOverrides needed.
...
/**
 * Hook invoked by DataSource.setupDataAccess before juggler registers its
 * ObjectId validations.
 *
 * Juggler stamps the connector's `getDefaultIdType()` (ObjectID) onto every
 * generated id property, then validates anything so typed as a 24-char hex
 * ObjectId. That id was typed by us rather than declared by the application,
 * and this connector's own isStoredAsObjectID() does not treat a
 * function-typed id as strictly-ObjectId, so the extra rule only rejects
 * documents the connector still stores correctly. Two cases break on it:
 *
 *  - `forceId: false`, where the flag exists precisely so callers may supply
 *    their own ids (numbers, short strings), and
 *  - `forceId: 'auto'`, where juggler's replaceById() only warns about a PK
 *    overwritten in a `before save` hook instead of restoring it (unlike the
 *    `loaded` path, which does restore), leaving the tampered value on the
 *    instance for isValid() to reject.
 *
 * Juggler exposes only a model-wide opt-out, so apply it only when the id is
 * the single ObjectId-typed property. A model that also declares its own
 * ObjectId properties keeps validation -- those were asked for explicitly, and
 * silencing them would be a real loss of coverage.
 */
MongoDB.prototype.define = function(modelDefinition) {
  // Register the model with the base connector first; it owns this._models,
  // which idName() and buildWhere() read.
  Connector.prototype.define.call(this, modelDefinition);

  const settings = modelDefinition.settings;
  const properties = modelDefinition.properties || {};

  if (settings.validateObjectIds !== undefined) return;

  const definition = modelDefinition.model && modelDefinition.model.definition;
  const idName = definition && definition.idName && definition.idName();
  const idProp = idName && properties[idName];

  // Only exempt an id juggler typed for us; a hand-declared ObjectId id means
  // the developer asked for that contract and should keep it.
  if (!idProp || !idProp.generated || idProp.type !== ObjectID) return;

  const hasOtherObjectIDProps = Object.keys(properties).some(
    (name) => name !== idName && isStoredAsObjectID(properties[name]),
  );

  if (!hasOtherObjectIDProps) {
    settings.validateObjectIds = false;
  }
};
  ...
  function doExecute() {
    ...
    self.notifyObserversAround(
      ...
      function(context, done) {
        ...
        // find returns a cursor synchronously — no Promise needed
        if (command === 'find') {
          args.pop();
          ...
          // All other commands are Promise-only in mongodb v7
          const cmdArgs = args.slice(0, args.length - 1);
          debug('MongoDB: model=%s command=%s', modelName, command, cmdArgs);
          collection[command].apply(collection, cmdArgs)
            .then(function(result) { observerCallback(null, result); })
            .catch(function(err) { observerCallback(err); });
        }
      },
      callback,
    );
...
MongoDB.prototype.multiInsertSupported = true;

/**
 * Create multiple model instances in a single bulk insert
 * @param {String} modelName The model name
 * @param {Object[]} dataArray Array of model data
 * @param {Object} options Options
 * @param {Function} callback Callback function (err, savedArray)
 */
MongoDB.prototype.createAll = function(modelName, dataArray, options, callback) {
  const self = this;
  if (self.debug) {
    debug('createAll', modelName, dataArray);
  }
  const idName = self.idName(modelName);

  const preparedArray = dataArray.map(function(data) {
    const cloned = Object.assign({}, data);
    const idValue = self.getIdValue(modelName, cloned);
    if (idValue == null) {
      delete cloned[idName];
    } else {
      cloned._id = self.coerceId(modelName, idValue, options);
      if (idName !== '_id') {
        delete cloned[idName];
      }
    }
    return self.toDatabase(modelName, cloned);
  });

  this.execute(modelName, 'insertMany', preparedArray, buildOptions({safe: true},
    options), function(err, result) {
    if (self.debug) {
      debug('createAll.callback', modelName, err, result);
    }
    if (err) return callback(err);

    const insertedIds = result.insertedIds || {};
    let coerceErr;
    const savedArray = preparedArray.map(function(data, i) {
      let idValue = data._id != null ? data._id : insertedIds[i];
      try {
        idValue = self.coerceId(modelName, idValue, options);
      } catch (e) {
        coerceErr = e;
      ...
      delete data._id;
      data = self.fromDatabaseToPropertyNames(modelName, data);
      data[idName] = idValue;
      return data;
    });

    if (coerceErr) return callback(coerceErr);
    process.nextTick(function() {
...
MongoDB.prototype.updateOrCreate = function updateOrCreate(
  ...
  this.execute(
    ...
    {_id: oid},
    data,
    buildOptions({upsert: true, returnDocument: 'after', includeResultMetadata: true}, options),
    function(err, result) {
      if (err) return callback(err);
      if (self.debug) {
      ...
      const isNewInstance = !(result && result.lastErrorObject && result.lastErrorObject.updatedExisting);
      const object = result && result.value;
      if (!object) {
        return callback('No ' + modelName + ' found for id ' + id);
      }
      self.setIdValue(modelName, object, oid);
      if (idName !== '_id') {
        delete object._id;
      }
      callback(null, self.fromDatabase(modelName, object), {isNewInstance});
    },
  );
};
...
MongoDB.prototype.findOrCreate = function findOrCreate(modelName, filter, data, options, callback) {
  ...
  // Coerce mongodb.dataType fields (e.g. ObjectID) before $setOnInsert, same as create().
  data = self.toDatabase(modelName, data);

  // findOneAndUpdate with $setOnInsert is atomic: if the document exists it is
  // returned as-is (no writes); if it doesn't exist it is inserted.
  // returnDocument:'after' gives the post-op document in both cases.
  // includeResultMetadata:true provides lastErrorObject.updatedExisting so we
  // can derive `created` without a separate round-trip.
  this.execute(
    modelName,
    'findOneAndUpdate',
    query,
    {$setOnInsert: data},
    buildOptions({upsert: true, returnDocument: 'after', sort, includeResultMetadata: true}, options),
    function(err, result) {
      if (err) return callback(err);
      if (self.debug) {
      ...
      const created = !(result && result.lastErrorObject && result.lastErrorObject.updatedExisting);
      const value = self.fromDatabase(modelName, result && result.value);
      if (value) self.setIdValue(modelName, value, value._id);
      if (value && idName !== '_id') delete value._id;

      if (filter && filter.include) {
        self._models[modelName].model.include([value], filter.include, function(err, included) {
          callback(err, included && included[0], created);
        });
      } else {
...
MongoDB.prototype.updateAttributes = function updateAttrs(
  ...
  this.execute(
    ...
    function(err, object) {
      if (self.debug) {
        debug('updateAttributes.callback', modelName, id, err, object);
      }
      if (!err && !object) {
        // No result
...
MongoDB.prototype.upsertWithWhere = function upsertWithWhere(
  ...
  // findOneAndUpdate with sort for atomicity — same semantics as the original.
  // includeResultMetadata:true is required at this site to derive isNewInstance:
  // v7 returns the document directly by default, stripping lastErrorObject.
  // This is a targeted per-call flag, not a global shim.
  self.execute(
    modelName,
    'findOneAndUpdate',
    where,
    updateData,
    buildOptions({upsert: true, returnDocument: 'after', sort: {_id: 1}, includeResultMetadata: true}, options),
    function(err, result) {
      if (err) return cb && cb(err);
      if (self.debug) debug('upsertWithWhere.callback', modelName, where, updateData, err, result);
      const object = self.fromDatabase(modelName, result && result.value);
      if (object) self.setIdValue(modelName, object, object._id);
      if (object && idName !== '_id') delete object._id;
      const isNewInstance = !(result && result.lastErrorObject && result.lastErrorObject.updatedExisting);
      if (cb) cb(null, object, {isNewInstance});
    },
  );
};
...
MongoDB.prototype.disconnect = function(cb) {
  ...
  */
  const closePromise = this.client ? this.client.close() : Promise.resolve();
  this.db = null;
  this.client = null;
  closePromise.then(function() { if (cb) cb(); }, function(err) { if (cb) cb(err); });
};
...
MongoDB.prototype.autoupdate = function(models, cb) {
  ...
  if (self.db) {
    ...
    // Serial across models (multiple models might map to the same collection)
    // and serial across indexes (avoid concurrent createIndex on the same collection).
    (async () => {
      for (const modelName of models) {
        ...
        for (const idx of indexList) {
          if (self.debug) {
            debug('createIndex: ', idx);
          }
          await self
            .collection(modelName)
            .createIndex(idx.fields || idx.keys, idx.options);
        }
      }
    })().then(() => cb(), (err) => cb(err));
  } else {
      ...
      for (const modelName of models) {
        ...
        try {
          ...
          if (
            ...
            // Rethrow anything other than "ns not found" (collection didn't exist).
            throw err;
          }
        }
...
MongoDB.prototype.ping = function(cb) {
  const self = this;
  if (self.db) {
    this.db.collection('dummy').findOne({_id: 1})
      .then(function() { cb(); })
      .catch(cb);
  } else {
    self.dataSource.once('connected', function() {
...
MongoDB.prototype.commit = function(tx, cb) {
  ...
  tx.commitTransaction()
    .catch(function(err) { commitErr = err; })
    .then(function() { return tx.endSession(); })
    .then(function() { cb(commitErr); })
    .catch(cb);
};
...
MongoDB.prototype.rollback = function(tx, cb) {
  let abortErr;
  tx.abortTransaction()
    .catch(function(err) { abortErr = err; })
    .then(function() { return tx.endSession(); })
    .then(function() { cb(abortErr); })
    .catch(cb);
};
